      ...
      <div className={`flex flex-col p-2 ${!expanded && 'items-center gap-4'} max-lg:hidden`}>
        <ul className='flex flex-col gap-4'>
          {userSideBars.map((item) => (
            <Link href={item.path} key={item.name} className={`flex-center w-full gap-4 rounded-md px-4 py-2 hover:bg-background hover:invert-[0.05] ${pathname == item.path && 'bg-background invert-[0.05]'}`}>
